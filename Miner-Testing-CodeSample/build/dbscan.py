...
class DbScan():
    ...
    def find_cluster(self, clusters):
        ...
        def find_first_point(source, cluster):
        ...
        def count_step(state, source, path):
            energy = state.energy

            path_to_des = path[source[0]][source[1]][::-1]
            ...
            gold_amount = self.gold_info[source[0], source[1]]
            while gold_amount > 0:
                if 5 >= energy:
                    for nang_luong_hoi in (4,3,2):
                        if energy < 40:
        ...
        for idx_clus, cluster in enumerate(clusters):
            ...
            point, idx_point = find_first_point(source, cluster)
            source = point
            order_point_in_cluster_.append(point)
            cluster.pop(idx_point)
            num_step = count_step(state, source, self.dbscan_path[point[0]][point[1]])
            gold = self.gold_info[point[0]][point[1]]
